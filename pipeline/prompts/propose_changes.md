...
- **A threshold no reality can meet.** The live value isn't near the threshold and there's no plausible path to it (`forward_pe < 3` against a live P/E of 19). Judge "no plausible path" against the `[history: …]` line when one is shown — the value range over recent sweeps is the evidence; a single reading is not. If a condition carries no history, prefer silence over extrapolation. Propose an `update` to a threshold that expresses the same intent — cheap relative to where the stock actually trades — not merely one that would pass today. A threshold nudged just past the current value is a rubber stamp, not a thesis.
- **A metric that never resolves.** The condition's `[history: …]` line shows `resolved 0/N` with N ≥ 5 — it has never produced a value, so it can only ever leave the thesis `incomplete`. One unavailable reading is a data blip, not a defect; without a history line showing repeated failure, do not propose this. Propose `remove`, or an `update` to a metric that does resolve.
...
Thesis: AAPL, quant `peg_ratio < 1.5` (id `q2`), live value UNAVAILABLE, `[history: resolved 0/10 recent sweeps — never produces a value]`, status `incomplete`.
→ `{target: "quant", action: "remove", target_id: "q2", rationale: "peg_ratio has come back unavailable in all 10 recent sweeps, so this condition only ever leaves the thesis 'incomplete' — it never contributes a pass or a fail. Removing it lets the remaining conditions decide.", confidence: 0.66}`
(Contrast: the same condition with `[history: resolved 9/10]` and one unavailable reading is a blip — no proposal.)
